...
fn main() {
    ...
    match args[1].as_str() {
        "-help" => print_usage(),
        "-monitors" => match mac::get_monitor_count() {
            Ok(count) => println!("Number of monitors: {}", count),
            Err(e) => eprintln!("Error querying monitors: {}", e),
        },
        "-size" => show_monitor_sizes(),
        "-change" => {
            // Collect remaining arguments for change command
            let change_args: Vec<String> = args.iter().skip(2).cloned().collect();
            change_background(&change_args);
        }
        _ => {
...
fn print_usage() {
    println!("Background Manager");
    println!("\nUsage:");
    println!("  background_manager -help                    Show this help message");
    println!(
        "  background_manager -monitors                Query the number of connected displays"
    );
    println!("  background_manager -size                    Show the pixel size of each monitor");
    println!("  background_manager -change <file>           Set the specified image as background on all monitors");
    println!("  background_manager -change <file> <monitor> Set the specified image on monitor #n");
}

fn show_monitor_sizes() {
...
fn change_background(args: &[String]) {
